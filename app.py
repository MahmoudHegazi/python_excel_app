...
def CreateNewDir():
    UPLOAD_FOLDER = os.path.join(app.root_path, 'upload_dir\\')
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            for worksheet in worksheetsList:
                ...
                # check if empty sheet contiue first continue
                if df.empty or len(df.columns) == 0 or len(dataframe.columns) == 0:
                    continue
                ...
                insertQuery += ";"
                createTableStatment = cursor.execute(queryString)
                insertDataQuery = cursor.execute(insertQuery)
                # Here Insert worksheets in worksheets tables MAIN TABLE
                sheetrealname = functions.riskVar(worksheet)
                insertWorkSheetQuery = "INSERT INTO worksheets (file_id, file_name, sheet_name, file_path, real_name) VALUES (%s, '%s', '%s', '%s', '%s') RETURNING id;"%(sheet_id, file_namedb, tablename, file_path, sheetrealname)
            ...
            success_message = 'Sheet Added To system successfully %s'%file_path
...
def mytest():
    ...
    sheetQuery = cursor.execute(insertSheetQuery1);
    """
    return str(allsheets)
    try:
        workbook=openpyxl.load_workbook('C:\\Users\\Mahmoud\\Downloads\\allvideos\\flask-app\\flask-app\\upload_dir\\2021-08-10\\file5_Inventory.xlsx')
        sheetnames = workbook.get_sheet_names()
        std=workbook.get_sheet_by_name('Spare SFPs')
        workbook.remove_sheet(std)
        sheetnames = workbook.get_sheet_names()
        workbook.save('C:\\Users\\Mahmoud\\Downloads\\allvideos\\flask-app\\flask-app\\upload_dir\\2021-08-10\\file5_Inventory.xlsx')
    except KeyError:
        return "Sheet Already Deleted"
    return str(sheetnames)
